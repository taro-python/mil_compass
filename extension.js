/* --- Tactical OS Extension v1.1: Storage & List Feature --- */

// 1. スタイルの注入（リスト画面とボタンの見た目）
...
extensionStyle.innerHTML = `
    ...
    #list-btn { bottom: 30px; left: 120px; padding: 12px 16px; } /* ミニコンパスの横 */
    ...
    .plot-info { font-size: 0.8rem; }
    .del-btn { background: #440000; color: #ff0000; border: 1px solid #ff0000; padding: 5px; cursor: pointer; margin-left: 10px; }
...
listPanel.innerHTML = `
    <h3 style="margin-top:0; border-bottom:1px solid #00ff00;">PLOT HISTORY</h3>
...
plotBtn.onclick = function() {
    ...
    const newPlot = { lat: center.lat, lng: center.lng, mgrs: mCode, dist: dStr, time: new Date().toLocaleString() };
...
window.deletePlot = function(index) {
    ...
    localStorage.setItem('tactical_plots', JSON.stringify(savedPlots));
    location.reload(); // 地図上のマーカーを消すためにリロード
};
...
console.log("Extension v1.1 Loaded.");
